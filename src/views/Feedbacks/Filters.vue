...
// coloca dados no padrão que será usado no template
function applyFiltersStructure (summary) {
  return Object.keys(summary).reduce((previousValue, currentValue) => {
    const currentFilter = {
    ...
    if (currentValue === 'all') {
      ...
      currentFilter.type = currentValue
    }

    return [...previousValue, currentFilter]
  }, [])
}
